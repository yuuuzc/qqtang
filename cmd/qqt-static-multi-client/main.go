package main

import (
	"encoding/json"
	"flag"
	"fmt"
	"os"

	"qqtang/internal/tooling/clientpatch"
)

func main() {
	path := flag.String("file", "", "prepared Core.dll path")
	check := flag.Bool("check", false, "verify the static patch without changing the file")
	output := flag.String("out", "", "optional JSON report path")
	flag.Parse()
	if *path == "" {
		fatalf("-file is required")
	}
	var (
		result clientpatch.MultiClientPatchResult
		err    error
	)
	if *check {
		result, err = clientpatch.CheckStaticMultiClient(*path)
	} else {
		result, err = clientpatch.PatchStaticMultiClient(*path)
	}
	if err != nil {
		fatalf("%v", err)
	}
	data, err := json.MarshalIndent(result, "", "  ")
	if err != nil {
		fatalf("encode result: %v", err)
	}
	data = append(data, '\n')
	if *output == "" {
		_, _ = os.Stdout.Write(data)
		return
	}
	if err := os.WriteFile(*output, data, 0o644); err != nil {
		fatalf("write result: %v", err)
	}
}

func fatalf(format string, values ...any) {
	fmt.Fprintf(os.Stderr, format+"\n", values...)
	os.Exit(1)
}
