...
func main() {
	...
	var (
		result clientpatch.PlatformEffectPatchResult
		err    error
	)
	if *check {
		result, err = clientpatch.CheckStaticPlatformEffects(*path)
	} else {
		result, err = clientpatch.PatchStaticPlatformEffects(*path)
	}
	if err != nil {
		fatalf("%v", err)
	}
