...
func main() {
	...
	var (
		result clientpatch.RoomPropertiesPatchResult
		err    error
	)
	if *check {
		result, err = clientpatch.CheckStaticRoomProperties(*file)
	} else {
		result, err = clientpatch.PatchStaticRoomProperties(*file)
	}
	if err != nil {
		fmt.Fprintln(os.Stderr, "qqt-static-room-properties:", err)
		os.Exit(1)
	}
